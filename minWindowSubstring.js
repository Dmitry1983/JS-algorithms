...
// will be strings ranging in length from 1 to 50 characters and all of K's characters will exist
...
const input = ["ahffaksfajeeubsne", "jefaa"];

const minWindowSubstring = (arr) => {
  const k = arr[0].split("");
  const n = arr[1].split("");
  return console.log({ arr, k, n });
};

minWindowSubstring(input);
