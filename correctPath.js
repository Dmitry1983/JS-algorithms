...
// direction to take within the grid, for example:
...
// all the way to the bottom right without touching previously travelled
...
const str = "???rrurdr?";
const str2 = "drdr??rrddd?";

const correctPath = (str) => {
  const arr = str.split("");
  let result = [...arr];
  const l = (arr, search) => {
    return arr.filter((item) => item === search).length;
  };

  for (let i = 0; i < arr.length; i++) {
    let horizontal = l(result, "r") - l(result, "l");
    let vertical = l(result, "d") - l(result, "u");
