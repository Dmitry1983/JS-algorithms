...
// strArr, which will contain characters ranging from A to Z in some arbitrary order,
//  and determine what elements still remain in a virtual cache that can hold up to
//   5 elements with an LRU cache algorithm implemented. For example:
//   if strArr is ["A", "B", "C", "D", "A", "E", "D", "Z"], then the
//   following steps are taken:

// (1) A does not exist in the cache, so access it and store it in the cache.

// (2) B does not exist in the cache, so access it and store it in the cache
// as well. So far the cache contains: ["A", "B"].
...
// separated by a hyphen. Therefore, for the example above your program should
// return C-A-E-D-Z.
...
const LRUCache = (strArr) => {
  ...
  for (let i = 0; i < strArr.length; i++) {
    let index = result.indexOf(strArr[i]);
    if (index !== -1) {
