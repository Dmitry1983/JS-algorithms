...
// where as in a geometric sequence, each term after the first is multiplied by some
...
function arithGeo(arr) {
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    if (i > 0) {
      tempArith[i] = tempArith[0] + i * (arr[1] - arr[0]);
    }
    tempGeo += arr[0] * (arr[1] / arr[0]) ** i;
  }
  if (arr.reduce((a, b) => a + b) == tempArith.reduce((a, b) => a + b)) {
    return "Arithmetic";
  }
  if (arr.reduce((a, b) => a + b) == tempGeo) {
    return "Geometric";
  }
